...
fn main() {
    ...
    for snack in snacks.iter() {
        match snack {
            ...
            Snack::Cookies(cookies) => println!("The sugary snack is {} cookies", cookies),
    ...
    println!("An apple costs ${}", snacks[0].price());
    if let Snack::Cookies(number) = snacks[1] {
        println!("{} cookies costs ${}", number, snacks[1].price());
    }
    if let Snack::Sandwich { lettuce, cheese } = snacks[2] {
...
impl Snack {
    fn price(&self) -> u8 {
        match self {
            Self::Apple => 5,
            Self::Cookies(cookies) => 2 * cookies,
