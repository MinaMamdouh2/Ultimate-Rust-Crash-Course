fn main() {
    ...
    let container: String = "box".to_owned();
    ...
    // Note
    // Using macros for vector makes the compiler optimize this better than creating a new vector and do some push calls
    // let things = vec![item, animal, container, material];

    let mut things: Vec<String> = Vec::new();
    things.push(item);
    things.push(animal);
    things.push(container);
    things.push(material);
