...
fn eat(s: String) -> bool {
    // This is the last expression of the function so it returned as the value of the function
    if s.starts_with("b") && s.contains("a") {
        true
    } else {
        false
    }
    // We can simplify more by only leaving this in this function this will evaluate to true or false and will be returned
